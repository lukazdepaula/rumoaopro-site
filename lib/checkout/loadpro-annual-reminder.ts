...
export function prepareAnnualTrialReminder(access:LoadProBillingAccess, options:{
  ...
  if (options.existingTrialReminder !== false || options.offerConsent!==true || options.suppressed!==false || options.deliveredKeys.has(key)
    || !plan || access.currency!=='BRL' || access.price_cents!==plan.monthlyCents
    || !access.user_id || !['active','trialing'].includes(access.status) || access.access_kind!=='subscription' || meta.provider_subscription_status!=='trialing'
    || meta.cancel_at_period_end===true || meta.annual_change || meta.billing_interval==='year'
    || !Number.isFinite(start) || !Number.isFinite(end) || end-start!==7*86400000 || start>now
    || left<=0 || left>48*3600000) return null;
  const link=new URL('/?view=setup&settings=security',options.appUrl);
  if (link.username || link.password || (link.protocol!=='https:' && !(link.protocol==='http:' && ['localhost','127.0.0.1'].includes(link.hostname)))) throw new Error('Invalid subscription URL');
  ...
  const preferences=new URL('/?view=setup&settings=notifications',link);
  preferences.searchParams.set('lang',en?'en':'pt');
